...
def get_summary_saver_hook(every_n_iter, tensors_to_log, model_dir, **kwargs):
    # tensors_to_log = {"total_loss": "total_loss",
    #                   "language_model_loss": "language_model_loss",
    #                   "next_sentence_loss": "next_sentence_loss"}
    # graph = ops.get_default_graph()
    # print([x for x in graph.as_graph_def().node])
    # tensors = [graph.as_graph_element(tensors_to_log[key]) for key in tensors_to_log]
    return tf.train.SummarySaverHook(save_steps=every_n_iter, scaffold=tf.train.Scaffold(summary_op=tf.summary.merge_all()), output_dir=model_dir)
...
HOOKS = {
    ...
    'summarysaverhook': get_summary_saver_hook,
}
